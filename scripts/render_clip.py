...
def fit_and_mux(video, audio, caption, out_mp4, ready=0.0, speed=1.0, still=False, scale=None, font_px=None, size=None):
    ...
    if size:  # common frame for every scene (e.g. 1440x900): fit inside, pad with black — lets terminal and browser scenes concat cleanly
        W, H = size.lower().split("x"); pre += f"scale={W}:{H}:force_original_aspect_ratio=decrease,pad={W}:{H}:(ow-iw)/2:(oh-ih)/2,"
    if still:
...
def main():
    ...
    for i, sc in enumerate(spec["scenes"]):
        ...
        fit_and_mux(v, a, sc.get("caption", ""), seg, ready, float(sc.get("speed", 1)), still, sc.get("scale"), sc.get("font"), spec.get("size")); parts.append(seg)
