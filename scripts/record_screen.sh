#!/usr/bin/env bash
# Record the screen while a command performs the action.   record_screen.sh <scene-dir> <seconds> [--rect x,y,w,h] -- <command…>
# --rect (points, not pixels) records only that rectangle — use it to capture one pane/window and nothing else on the screen.
...
set -uo pipefail
D=$1; SECS=$2; shift 2; RECT=(); if [ "${1:-}" = "--rect" ]; then RECT=(-R"$2"); shift 2; fi; [ "${1:-}" = "--" ] && shift
mkdir -p "$D"; rm -f "$D"/page.mov "$D"/marks.json; sleep 2
screencapture -x "${RECT[@]}" -V "$SECS" "$D/page.mov" & CAP=$!; T0=$(python3 -c 'import time;print(time.time())'); sleep 1.5
started=""
